...
class Dispenser
{
    ...
    {
        echo "Volume air galon yang dimiliki {$this->volume} ml <br> Saldo yang dimiliki {$this->jumlah_uang}<br> 
        Jumlah gelas yang dimiliki {$this->jumlah_gelas} <br> volume gelas yang dimiliki {$this->volumeGelas}<br>";
    }
    ...
    public function isiUlangGalon(int $volume)
    {
        if ($volume > 0) {
            $this->volume = $volume;
            echo "Anda telah mengisi volume air galon sebesar {$this->volume} ml <br>";
        } else {
            echo "Volume air tidak bisa kurang dari 0 <br>";
        }
    }
    public function isiGelas()
    {
        if ($this->volume >= $this->volumeGelas && $this->jumlah_gelas > 0) {
            ...
            echo "Anda telah menuangkan air kedalam gelas berukuran {$this->volumeGelas} ml dengan harga {$this->hargaSegelas} <br>";
        } elseif ($this->jumlah_gelas <= 0) {
            echo "Tidak ada gelas tersedia, silahkan isi ulang gelas <br>";
        } elseif ($this->volume < $this->volumeGelas) {
            echo "Volume air galon tidak mencukupi <br>";
        }
    }
    ...
    public function isiUlangGelas(int $gelas)
    {
        ...
        if (
            ...
            echo "Anda telah melakukan isi ulang gelas sebanyak {$gelas} gelas dengan total harga {$total_harga} atau {$harga_gelas}/pcs <br>";
        } elseif ($this->jumlah_uang < $harga_gelas) {
            echo "Uang anda tidak mencukupi <br>";
        } elseif ($total_harga < $harga_gelas) {
            echo "Masukkan angka gelas dengan benar <br>";
        }
    }
}
...
class Minuman extends Dispenser
{
    ...
    {
        echo "Ini adalah {$this->namaMinuman} seharga {$this->hargaSegelas} per gelasnya <br>";
    }
}
